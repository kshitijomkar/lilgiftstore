...
export default function OrderStatusTimeline({ currentStatus, timeline = [] }: any) {
            ...
            <div className="relative">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  isCompleted ? 'bg-[#b96a82] text-white' : 'bg-gray-200 text-gray-400'
                } transition-all`}
              >
                <Icon className="w-5 h-5" />
              </div>
              {index < STATUS_ORDER.length - 1 && (
                <div className={`absolute top-10 left-5 w-0.5 h-16 ${isCompleted ? 'bg-[#b96a82]' : 'bg-gray-200'} transition-all`} />
              )}
            </div>

            <div className="flex-1 pt-1">
              <p className={`font-medium ${isCompleted ? 'text-[#4b2e2b]' : 'text-[#4b2e2b]/40'}`}>
                {config.label}
                {isCurrent && <span className="ml-2 text-xs bg-[#b96a82] text-white px-2 py-1 rounded-full">Current</span>}
              </p>
            </div>
          </div>
        );
